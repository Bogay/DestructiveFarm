...
def check_script_source(source: str, interpreter: Optional[str]):
    errors = []
...
def load_attack_chains(config_path: str) -> List[AttackChain]:
    config = yaml.safe_load(open(config_path).read())
    chains = [
        AttackChain(
            problem_name=ch["problem_name"],
            exploits=[Exploit(path=s) for s in ch["exploits"]],
        )
        for ch in config["chains"]
    ]

    for chain in chains:
        for exploit in chain.exploits:
            if not os.path.isfile(exploit.path):
                raise ValueError("No such file: {}".format(exploit.path))

            extension = os.path.splitext(exploit.path)[1].lower()
            is_script = extension in SCRIPT_EXTENSIONS
            if is_script:
                with open(exploit.path, "r", errors="ignore") as f:
                    source = f.read()
                errors = check_script_source(source, None)

                if errors:
                    logging.error(
                        f"Invalid exploit {exploit.path}@{chain.problem_name}"
                    )
            ...
            if not os_windows:
                file_mode = os.stat(exploit.path).st_mode
                # TODO: May be check the owner and other X flags properly?
                if not file_mode & stat.S_IXUSR:
                    if is_script:
                        logging.info(
                            "Setting the executable bit on `{}`".format(exploit.path)
                        )
                        os.chmod(exploit.path, file_mode | stat.S_IXUSR)
                    else:
                        raise InvalidSploitError(
...
def post_flags(args, flags: List["FlagInfo"]):
    data = [
        {
            "flag": item.flag,
            "sploit": item.exploit,
...
class FlagStorage:
    ...
    def mark_as_sent(self, count: int):
        with self._lock:
...
def display_sploit_output(team_name: str, output_lines: List[str]):
    if not output_lines:
...
def launch_sploit(
    ...
    command = [os.path.abspath(exploit.path)]
...
def run_sploit(
    ...
    for exploit in chain.exploits:
        if not exploit.valid:
            logging.warning(f"Invalid exploit {exploit.path}@{chain.problem_name}")
            continue
        ...
        try:
            with instance_lock:
                if exit_event.is_set():
                    return
                proc, instance_id = launch_sploit(exploit, team_addr)
        except Exception as e:
            if isinstance(e, FileNotFoundError):
                logging.error(
                    "Sploit file or the interpreter for it not found: {}".format(
                        repr(e)
                    )
                )
                logging.error(
        ...
        try:
            ...
            try:
                ...
                elapsed_time = time.time() - starts_at
                max_runtime -= elapsed_time

                is_attack_success = process_sploit_output(
                    proc.stdout,
                    args,
                    team_name,
                    flag_format,
                ...
                need_kill = True
                logging.warning(
                    f'[round {attack_no} / {exploit.path}] Sploit for "{team_name}" ({team_addr}) ran out of time'
                )
...
def main(args):
    ...
    for attack_no in once_in_a_period(args.attack_period):
        ...
        chains = load_attack_chains(args.config)
